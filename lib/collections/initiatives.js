...
var sendFollowersNotification = function sendFollowersNotification(doc, type, currentUserId) {
  ...
  _.each(doc.usersFollowing, function eachUser(userId) {
    ...
    createNotification(doc, type, currentUserId, userId);
  });
};
...
Initiatives.after.update(function initiativesAfterUpdate(userId, doc, fieldNames, modifier) {
  ...
  if (modifier.$addToSet && modifier.$addToSet.comments) {
    if (doc.createdBy !== userId) {
      createNotification(doc, 'comment', userId, doc.createdBy);
    }
    sendFollowersNotification(doc, 'comment', userId);
  }

  if (modifier.$addToSet && modifier.$addToSet.usersVoted) {
    createNotification(doc, 'vote', userId, doc.createdBy);
    sendFollowersNotification(doc, 'vote', userId);
  }

  if (modifier.$pull && modifier.$pull.usersVoted) {
    createNotification(doc, 'remove-vote', userId, doc.createdBy);
  }

  if (modifier.$addToSet && modifier.$addToSet.usersFollowing) {
    createNotification(doc, 'follow', userId, doc.createdBy);
    sendFollowersNotification(doc, 'follow', userId);
  }

  if (modifier.$pull && modifier.$pull.usersFollowing) {
    createNotification(doc, 'unfollow', userId, doc.createdBy);
    sendFollowersNotification(doc, 'unfollow', userId);
  }
});