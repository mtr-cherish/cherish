...
Meteor.methods({
  createNotification: function createNotification(doc, type, ownerId) {
    var userId = Meteor.userId();

    Notifications.insert ({
      initiativeId: doc._id,
      userId: userId,
      ownerId: ownerId !== undefined ? ownerId : doc.ownerId,
      type: type,
  ...
  sendFollowersNotification: function sendFollowersNotification(doc, type, ownerId) {
    ...
    _.each(doc.usersFollowing, function eachUser(userId) {
      ...
      Meteor.call('createNotification', doc, type, ownerId);
    });
  }
});