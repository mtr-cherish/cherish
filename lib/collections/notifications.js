...
Meteor.methods({
  ...
  sendFollowersNotification: function sendFollowersNotification(doc, type, notificationOwnerId) {
    if (!doc.usersFollowing || doc.usersFollowing.length === 0) {
      return;
    }
    _.each(doc.usersFollowing, function eachUser(followerId) {
      if (followerId === notificationOwnerId) {
        return;
      }
      Meteor.call('createNotification', doc, type, followerId, notificationOwnerId);
    });
  }
});